...
politicas:
  ...
  - v1 publica apenas no Instagram (o campo `rede` do perfil fica fixo em
    `instagram`).
  ...
  - O parâmetro `--entrada` é opcional. Quando omitido, o tema do post é
    escolhido pelo próprio agente a partir de `pesquisar_tendencias_nicho`, nunca
    inventado sem base de pesquisa — essa é a diferença entre um agente
    `goal_oriented` de verdade e um `task_based` disfarçado.
  - A ferramenta `autocritica_conteudo` é obrigatória em roteiro e em visual, e seus
  ...
  - hoje `publicar_conteudo` sempre passa por confirmação humana (nesta versão,
    você publica manualmente com a peça aprovada, ou aprova a publicação
